...
class SimpleDeliveryRobot(Node):
    ...
    def timer_callback(self):
        ...
        elif self.goal_status == 5 or (self.goal_status == 4 and self.task_cancled):
    ...
    def handle_task_cancellation(self):
        ...
        self.get_logger().info(f"Task Canceled! Taking appropriate action. {self.goal_status}")
        if self.current_goal == "kitchen":
            self.get_logger().info("Task canceled. Returning home.")
            ...
            self.get_logger().info("Task canceled. Returning to kitchen first, then home.")
            self.waiting_for_confirmation = False
            self.pending_orders.clear()
    ...
    def handle_confirmation_timeout(self):
        ...
        if self.confirmation_timeout <= 0:
            ...
            if self.current_goal == "kitchen":
                ...
                self.pending_orders.clear()
                self.send_goal("home")
            elif "table" in self.current_goal:
        ...
        try:
            ...
            if result:
                ...
                if result.status == 4:
                    ...
                    self.get_logger().warning(f"{self.current_goal} Canceled!")
                    self.pending_orders.clear()
                    self.task_cancled = True
