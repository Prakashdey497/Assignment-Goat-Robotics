...
class SimpleDeliveryRobot(Node):
    ...
    def timer_callback(self):
        ...
        elif self.table_skiped and not any("table" in order for order in self.pending_orders):
            self.pending_orders = ["kitchen", "home"]
            self.table_skiped = False
            
        elif self.current_goal == "kitchen"  and self.table_skiped:
            self.get_logger().info("Goal Cancle. Returning home.")
    ...
    def handle_confirmation_timeout(self):
        ...
        if self.confirmation_timeout <= 0:
            ...
            elif "table" in self.current_goal and self.table_skiped:
